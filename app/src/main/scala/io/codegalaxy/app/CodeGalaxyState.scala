...
import io.codegalaxy.app.question.{QuestionState, QuestionStateReducer}
...
case class CodeGalaxyState(currentTask: Option[AbstractTask],
                           ...
                           questionState: QuestionState) extends CodeGalaxyStateDef
...
object CodeGalaxyStateReducer {
  ...
  def reduce(state: Option[CodeGalaxyState], action: Any): CodeGalaxyState = CodeGalaxyState(
    ...
    questionState = QuestionStateReducer(state.map(_.questionState), action)
  )
